...
- **AI 查询准确率**：从 `asset_query` 调用日志 + F4.1 反馈抽样，意图识别 + 参数提取 + 结果正确的占比；目标 ≥80%；W0 评测集作为回归基线。**基线已录（2026-08-22）：49/50 = 98%（intent-only，glm-4-flash），对抗样本拒答 5/5**；重跑命令见 `scripts/eval_asset_query.py`。
