...
## 今日补充（2026-09-14：资产知识图谱 graph builder 阻塞根治 + 后端全站 500 恢复）

### 背景
图谱代码已在 `86dd48c` 全量落地但无法演示：开启 `GRAPH_BUILDERS_ENABLED=true` 后，
64571 条 identity_events 的 IdentityGraphBuilder **同步跑在 uvicorn 单 worker 的
event loop 线程里**，把全站 API（连 /health、/public）拖到超时（HTTP 000/500）。
临时用 `GRAPH_BUILDERS_ENABLED=false` 启动保 API，但 builder 永远不跑、图谱无真实边。
本轮把 builder 改成可安全常开。

### 根因（三层）
1. **阻塞 event loop**：scheduler 的 6 个 `@track_task async def` 内部直接调同步
   `builder.rebuild_all()`（重 DB/CPU），没经 `asyncio.to_thread`，单 worker 被占死。
2. **scheduler task 从不 commit**：`rebuild_all()` 只 `flush()`，6 个定时 task 用
   `SessionLocal()` 后只在 finally `db.close()`（隐式回滚）→ **数据根本不落库**，
   异常路径还留 `idle in transaction` 僵尸事务（PG 端可见 ClientRead 卡几分钟）。
3. **IdentityGraphBuilder 有必崩 bug**：`upsert_edge(self.db, _now=_t_now, **e)`
   而 `e` 字典本身已含 `"_now"` 键 → `got multiple values for keyword argument '_now'`，
   在 session_on 阶段必抛异常。**identity builder 历史上从未成功跑完过一次**。

### 修复（`app/services/graph/scheduler.py` + `api/graph.py` + `builders.py`）
- 新增 `_run_builder_in_thread(fn, name, wait=...)`：**工作线程内**建 Session、
  try commit / except rollback / finally close，杜绝僵尸事务。
- 所有 builder 经 `asyncio.to_thread(...)` 跑，uvicorn 主循环照常服务 HTTP。
- 对外入口 `run_builder_async(fn, name, raise_on_error, wait)`，定时循环与
  `POST /graph/rebuild` 端点共用。
- **串行锁关键决策：用 `threading.Lock` 且在线程内获取，不能用 `asyncio.Semaphore`**——
  `@track_task` 的 timeout 会 cancel 协程，asyncio 锁随 cancel 提前释放，但
  `to_thread` 的工作线程不可中断仍在写库，导致下一个 builder 进临界区、两个全量重建
  在 `soc_graph_nodes` 行锁互等（`Lock/transactionid`）。threading.Lock 随线程生命
  周期严格持有。
  - 高频周期任务（identity/alert/manual/cleanup）`wait=False`：抢不到锁**本轮 skip**
    （下轮再补），不堆积不超时；
  - 每日 cron（asset_port_vuln/topology）`wait=True`：撞车排队，保证当天必跑；
  - HTTP `/rebuild` `wait=True` + `raise_on_error=True`（返回 500）。
- `run_all_builders(db, only=...)` 新增 only 参数支持单 builder（复用其 per-builder
  commit/rollback 保护）；`/rebuild` 端点改为 to_thread + 独立 Session（不再占请求级
  get_db session），清掉 5 个 unused Builder import。
- 修 `_daily_loop` 月末 bug：`target.replace(day=now.day+1)` → `+timedelta(days=1)`。
- identity timeout 300→900s、topology 600→900s（实测 topology 356s）。
- 修 builders.py:512 `_now` 重复关键字参数（去掉显式那个，dict 里已有）。

### 实测（本地 testdb，生产未动）
- identity **首次成功**：events 64571 → login_to 56 / login_from 15 / external_access
  1888 / session_on 24 / owned_by 3，195s。
- topology：same_segment 2820 / shared_tag 43，356s。
- 最终 **1826 节点 / 3748 边**落库（11 类关系）。
- 重建全程并发 curl /health、/public：**0.1–0.6s 稳定 200/即时返回**（health=503
  是既有 browsing_detector/loki 降级业务码，非超时/崩溃），改造前是全站 000。
- 启动首跑：identity 抢锁，alert_group/manual/cleanup 全部
  `skipped: another builder is running`，无并发、无锁等待。
- builder 跑完后 PG 连接全部 COMMIT/ROLLBACK 归还（xact=None），
  **零 idle in transaction 残留**。
- `.env` 恢复 `GRAPH_BUILDERS_ENABLED=true` 并常开（现可安全开启）。

### 踩坑（本轮）
1. **kill uvicorn 进程不会终止 PG 后端线程**：上一进程被 kill 后，其 builder 的
   未提交事务成孤儿，继续持锁，新进程 builder 被它阻塞（`Lock/transactionid`）。
   排障脚本 `/tmp/kill_orphan_graph.py`：只杀 >4min 的老 idle-in-transaction + 老
   lockwaiter，保留新进程连接。重启前最好先确认 PG 无 graph 孤儿。
2. **asyncio.to_thread 的位置参数**：`to_thread(f, a, b, wait)` 会把 wait 也当位置参
   传，而签名是 keyword-only（`*, wait=True`）→ takes 2 positional but 3 given。
   必须 `to_thread(f, a, b, wait=wait)`。
3. **`@track_task` 已自带 per-task lock + orphan-thread 等待语义**，外面再叠 asyncio
   锁会冲突；跨 builder 互斥要在线程层做（见上 threading.Lock 决策）。
4. **别只数 HTTP 状态**：builder 卡死时 /health 也 000；恢复后 /health 返 503 是
   browsing_detector 降级（既有问题），接口是通的。看响应时间比看状态码更准。
5. **psycopg2 脚本里 load_dotenv() 在 stdin heredoc 下 find_dotnet 报 AssertionError**
   （无调用帧）——写成独立 .py 文件再跑（/tmp/*.py）。连库用 settings.DB_* 字段拼，
   Settings 没有 database_url 属性。
6. health 偶发 1–2s 尖峰发生在 builder 大批量 `ensure_nodes_batch` INSERT 时
   （1746 nodes 一把 upsert，行锁 + GIL），单次可恢复，非卡死；要更平滑可后续把节点
   batch 分批（独立性能工单，不影响安全性）。

### 遗留（不阻塞）
- topology/identity 慢在 `upsert_edge` 每条 1 SELECT + 1 INSERT（2N 次 RTT）；
  2863 边 ≈ 6 分钟。已被 to_thread 隔离不影响 API，但可批量化进一步提速（独立工单）。
- health=503 的 loki:browsing_detection 不可达是既有问题（192.168.0.30:3100），与本轮无关。
- 前端 `/assets/graph` 已重写为原型左中右三栏（另一工作线）；现真实边数据已就位，
  种子资产邻居子图应能出图，待浏览器最终确认。

---

## 今日补充（2026-09-14：P0 数据补齐地基落地 — F9/F10/F11）

### 背景
用户指示「推进 §7 数据补齐」的执行。经评估 12 项功能工作量(~5+ 天)+ 业务访谈 1h workshop 依赖,不可能一次性推完,
**先做 P0 地基**(低成本高 ROI,半天搞定 + 端到端验证),其余 F1~F8 + 业务访谈作为独立工单。

### 现状核实先于实施（教训：方案文档基线已过期）
- DB 实测 vs 文档「事实」列:
  - agent 纳管率:文档说 30% (52 盲区)，DB 实测 **19% (63/78 盲)** — 远差于文档
  - 业务系统数:文档说 0，实测 **3**(soc-platform/core-biz/identity-mgmt)
  - 业务系统关联:文档说 0%，实测 **6 条**
  - identity_bindings:文档说 0，实测 **24**
- 「事实」列有些过时，但 P0 项的差距定性仍正确——**先核实再开工**。

### F12「全局图谱页路由」非真问题（重要修正）
- 文档 §7.2.7 Gap B 写「asyncRoutes 无 graph 子路由、routesAlias 无 AssetGraph 别名 → 菜单有种子但点菜单 404」
- **事实**:art-design-pro-edge 是**后端驱动菜单**(`RouteTransformer` 用 `ComponentLoader.load(component)` 自动解析),
  前端业务路由由 `/api/v1/menus/tree` 推过来,**不需要**在 `asyncRoutes.ts` 手动注册
- CDP 实测:侧栏已有「资产图谱」菜单项，点击后 URL → `http://localhost:5173/#/assets/graph`,
  面包屑 `资产管理 / 资产图谱`，echarts canvas 渲染成功，**0 个 404**
- 教训：**写代码前对文档「事实」也需实测**，文档本身可能基于错误前提

### 交付（commit 待生成，本轮 4 文件改动）

#### F11 Asset.parent_id 模型对齐 DB
- DB 已是 UUID + FK self-reference（迁移 t3u4v5w6x7y8 已改），但 ORM 仍写 `parent_id = Column(String)`
  → alembic autogenerate 会误判产生 drop+recreate 噪音
- 模型改为 `Column(UUID(as_uuid=True), ForeignKey("soc_assets.id", ondelete="SET NULL"), nullable=True)`
- `alembic check` 已不再报 parent_id 相关 diff
- **无迁移、无 DDL 改动**——纯模型声明对齐

#### F9 业务系统 CRUD API（**真正缺失**）
**新增**:
- `src/backend/app/schemas/business_system.py`（~120 行）:
  - `BusinessSystemBase/Create/Update/Response/ListResponse`
  - `AssetBusinessLinkCreate/Response`（资产-系统关联）
  - code 正则约束（小写英文/数字/-/_，2~50），criticality 4 档枚举
- `src/backend/app/api/business_systems.py`（~250 行，8 个端点）:
  - `GET /` 列表（分页 + name/code 模糊 + 批量统计 asset_count 防 N+1）
  - `POST /` 新建（admin only，code 唯一性校验）
  - `GET /{id}` 详情
  - `PUT /{id}` 更新（admin only，Pydantic exclude_unset 局部更新）
  - `DELETE /{id}` 删除（admin only，级联清理关联）
  - `GET /{id}/assets` 系统下资产列表
  - `POST /assets/{asset_id}/systems` 关联资产到系统
  - `DELETE /assets/{asset_id}/systems/{system_id}` 解绑
- 全部走 `@log_audit` → 进 soc_audit_logs 哈希链；admin-only 写操作走 `require_admin()`
- 注册到 `app/api/__init__.py` 的 `include_human_router(api_router, business_systems.router, prefix="/business-systems", tags=["业务系统管理"])`

#### F10 AssetUpdate 拓归属字段 + 详情页暴露
- `schemas/asset.py`:
  - `AssetCreate/Update` 加 `owner_id`(Integer FK soc_users)、`department_id`(BigInteger FK soc_departments)
  - `AssetResponse` 加同两字段 + `business_system_names`(可选,仅详情填)
  - **不加** `business_system_ids`:业务系统多对多走独立 API（避免双源写入，§7.1.2 设计的确认工作台是单一入口）
- `api/assets.py` `get_asset/update_asset` 在序列化后追加 `business_system_names`（避免 list 端点 N+1）
- 前端 `types/api/api.d.ts` `AssetListItem` 加 3 个可选字段
- 前端 `views/asset/detail/index.vue` 详情页 ElDescriptions 新增 3 行:
  - 「归属业务系统」（多 ElTag，空时"未关联"）
  - 「责任人 ID」「部门 ID」（`?? '--'`）
- **不做编辑表单**：F4 确认工作台才是单一录入入口（避免多源写入冲突）

### 端到端验证（curl + CDP 双重）

#### F9 业务系统 CRUD（端到端通过）
```
GET 列表 → 200 (3 条种子：soc-platform/core-biz/identity-mgmt)
POST 新建(code=p3-test-biz) → 201 + UUID
GET 详情 → 200 (name/criticality/owner 全显)
PUT 更新(criticality=high) → 200 (新值生效)
POST 资产关联 → 201 (link role=web)
GET 系统下资产 → 200 (1 个资产+role)
GET 审计日志 resource_type=business_system → 5 条(CREATE/UPDATE/DELETE 哈希链落库)
DELETE → 200 (清理完成 + 解绑关联)
```

#### F10 资产归属（端到端通过）
```
PUT /assets/{id} owner_id=60 department_id=3 owner="F10 verify" → 200
   响应: owner_id=60, department_id=3, business_system_names=[]
GET /assets/{id} → owner_id 字段返回 60
回滚: owner_id=null → 200, owner=None
前端 CDP 实测：详情页 24 个 ElDescriptionsItem,「归属业务系统」「责任人 ID」「部门 ID」3 行均渲染
```

### 与 §7 实际差距分析（评估是否继续推 F1~F8）
| 工单 | 现状 | 工作量 | ROI |
|---|---|---|---|
| F1~F3 推断引擎+候选表 | 完全缺失（无 attribution/ 字样） | 1.5~2 天 | 高（自动从 soc_identity_events 猜 owner,72 台全有数据） |
| F4~F6 确认工作台 | 完全缺失 | 1.5~2 天 | 高（让业务/运维确认低置信项） |
| F7~F8 完整度评分卡 | 缺失（coverage.py 只算图拓扑） | 0.5 天 | 中（演示进度用） |
| WO-0f 提升 agent 纳管率 | 19%(63 盲) | 1~3 天 | 中（盲区资产边都是空） |
| WO-0d 端口覆盖 | 已 100%（74 全有端口） | 已完成 | - |
| WO-0g 登录规则面拓宽 | 6 类 → 加 Win 4624/4625 等 | 0.5 天 | 中 |
| 业务访谈(WO-0a workshop) | 核心系统圈定 | 1h 业务时间 | 必做 |

**建议下一步**：等用户 1h workshop 输出 5~10 个核心业务系统名字 → 一次性 seed 进 DB → 启 F1 推断引擎
自动猜 owner/业务归属 → F4 工作台人工复核低置信项 → 闭环 §1.3 三个用户故事。

### 踩坑（本轮新增）
1. **alembic check 噪音 ≠ 真问题**：env.py 只 import 4 个 model,其他表的注释/约束 diff 全是「模型未加载到 metadata」导致。
   grep `parent_id` 验证 F11 改对了,其他 diff 不用管(CLAUDE.md 既有教训「索引命名约定类 diff pre-existing 噪音,CI advisory 不阻塞」)
2. **后端冷启动 8s+**：sleep 8 还可能不够,scheduler + CISA KEV + Push + 各 graph watchdog 全部启动后才 listen。
   验证脚本要么 sleep 10+,要么轮询 health 200
3. **art-design-pro-edge 是后端驱动菜单**：别被「找不到 asyncRoutes 注册」吓住,去看 menus.py 推过来的 component 路径
4. **audit_decorator lambda 签名 `(result, kwargs)`**：UPDATE 端点直接返回 response_model 即可,不要返回 dict 兼容,
   参考 system_configs.py 风格;`get_resource_name` 在 DELETE 里要从 result dict 拿(装饰器看不到局部变量)
5. **`business_system_names` 加载**：list 端点不要 N+1,只在 get/update 单资产时查(74 个 → 单查)

---

## 今日补充（2026-09-14：业务系统管理界面与菜单落地 — F9 收尾）

### 背景
用户指出了一个交付缺口：**「业务系统有管理功能界面吗？配了菜单吗？」**——上一轮 P0 只完成 API,
前端页面与菜单均未创建。实质是「只有 admin curl 才能用,跟没建差不多」。

### 现状核实（三问三答）
- `soc_menus` 表里 `business` 关键字搜索 **0 条**记录 → 未配菜单
- `src/frontend/src/views/` 与 `src/api/` 下无 business 目录/封装 → 无前端页面
- admin 角色菜单清单 45 条中无「业务系统管理」→ 侧栏不可点

### 交付（3 新增文件 + 1 迁移 + 1 类型扩展）

**1. 前端 API 封装** `src/frontend/src/api/businessSystem.ts`（~110 行）
- 8 个包装函数：list/detail/create/update/delete/getSystemAssets/linkAsset/unlinkAsset
- 走与 api/department.ts 同风格的 `request.get/post/put/del`

**2. 前端管理页** `src/frontend/src/views/system/business-system/index.vue`（~300 行）
- ArtSearchBar 顶部搜索 + ArtTableHeader 工具栏 + ElTable 列表 + ElDialog 表单弹窗
- 8 列：编码/名称/重要性(彩色 tag)/责任人/部门/关联资产/描述/操作(编辑+删除)
- 表格按重要性逆序排列(严重→高→中→低)
- 重要性表单 + 严格 code 格式校验（中文/错位都拦）
- 删除带 ElMessageBox 确认 + 提示影响资产数

**3. 类型扩展** `src/frontend/src/types/api/api.d.ts`
- 新增 `namespace BusinessSystem { BusinessSystemItem / Payload / SearchParams }`

**4. 菜单迁移** `src/backend/alembic/versions/u5v6w7x8y9z0_business_system_menu.py`
- `down_revision='t3u4v5w6x7y8'`（head）
- 子菜单挂 `/system` 下，sort_order=8，icon=`ri:briefcase-line`，
  component=`/system/business-system/index`，permissions=[add/edit/delete]
- 纯 SQL INSERT…SELECT + NOT EXISTS 幂等（**不硬编码 parent_id**，按 path JOIN）
- 授权：admin 三按钮全授 + operator/viewer/auditor 空 perms（只读，UI 不显示按钮）
- `soc_role_menus` 无 created_at/updated_at 列（CLAUDE.md 既有教训）
- downgrade：按 path 定位菜单 + 角色授权，一句删两边（不遗留）

### 端到端验证
```
alembic upgrade head → 1 menu + 4 role auths
downgrade -1 → 0/0 → upgrade head → 1/4 ✓ 幂等
GET /business-systems → 200 (4 条种子数据正常返回)
vite 编译业务系统页 → 51KB,无语法错
admin 浏览器跳转 /#/system/business-system → 200
表格 + 新建按钮 + 搜索栏 + 分页器全部渲染
面包屑 「系统管理 / 业务系统管理」✓
侧栏「业务系统管理」图标 ri:briefcase-line ✓
顶部 tab 自动追加「业务系统管理」标签 ✓
```

### 踩坑（本轮新增）
1. **菜单 click 折叠态不可点**：第一轮 `.el-menu-item` click 跳 500（其实是折叠态元素不可点）,
   不是页面 bug。改用直接 `location.hash='#/system/business-system'` 跳转验证最稳
2. **ComponentLoader 用 vite import.meta.glob 静态扫**：路径与文件路径一一对应,
   菜单 component `/system/business-system/index` → `@/views/system/business-system/index.vue` 自动命中
3. **downgrade 要不遗留**：仅删 menu 会留孤儿 role_menus（FK 报 issue），必须先删 role_menus 再删 menu

### 待办（独立工单，下次可推进）
- F1~F8 + 业务访谈 workshop（~3~4 天）
- WO-0f agent 纳管（需运维配合）
- 前端归因工作台页面（依赖 F4）
- 全局图谱页加完整度评分卡卡片（依赖 F8）
- 业务系统管理页:点击行进详情页(资产列表) + 跨页批量操作(独立工单)
- 同步重建现有 3 个业务系统的归一化字段(从 soc_business_systems 看 description 等填充)

---

## 今日补充（2026-09-14：业务系统管理页重构 — 整体对齐角色管理页）

### 用户反馈的三个 UI 问题
1. 名称/编码列**换行**
2. 表格与右上角一排按钮（刷新/列设置/全屏）**间距几乎挨着**
3. 整体 UI 及元素间距**要与角色管理页（/system/role）保持一致**

### 根因（不是调某个间距能解决的）
第一版用的是**裸 `ElTable` + 手写 `ElPagination` + 手写列定义 + 手写 hasAuth**，
而角色页走的是项目标准封装 **`useTable` composable + `ArtTable` 组件**：
- `ArtTable` 通过 `:layout="{ marginTop: 10 }"` 提供「表格与 header」的间距（问题 2 的答案）
- `ArtTable` + `table-layout="fixed"` + 列 `showOverflowTooltip` 自动省略（问题 1 的答案）
- `useTable` 接管分页/加载/搜索/列设置，操作列用 `h(ArtButtonTable)` 图标按钮（问题 3 的答案）

**教训：新建 CRUD 页必须套 `useTable` + `ArtTable` 标准模式，不要裸用 ElTable**——
裸 ElTable 会丢掉项目封装好的间距/分页/列设置/省略号等所有一致性。

### 交付（重写 1 文件 + 调整 1 API）
- `views/system/business-system/index.vue` 整体重写（~300 行），完全对齐 `views/system/role/index.vue`：
  - `useTable<any>`：`apiFn=fetchBusinessSystemList`，`columnsFactory` 8 列（编码/名称/重要性/责任人/部门/关联资产/描述/操作）
  - 重要性列 `h(resolveComponent('ElTag'))` 彩色 tag；操作列 `h(ArtButtonTable)`（edit/delete 图标按钮）
  - 顶部搜索 `ArtSearchBar`（keyword 模糊）、`ArtTableHeader v-model:columns`、`ArtTable :layout="{marginTop:10}"`
  - 弹窗 ElDialog（编码/名称/重要性/责任人 ID/部门 ID/描述），严格 code 格式校验 + 删除 ElMessageBox 确认
- `api/businessSystem.ts` `fetchBusinessSystemList` 改返回**原始 envelope**（不再手动解包改名 records），
  配合 useTable 的 `transform.responseAdapter` 解 `res.data.items`

### useTable 适配的两个坑（本轮新增）
1. **分页字段名映射**：后端用 `page`/`page_size`，useTable 默认发 `current`/`size`。
   解法：`core.paginationKey = { current:'page', size:'page_size' }`——发请求字段名即对齐后端，
   无需 `normalizePaginationParams` 转换。
2. **响应提取**：`defaultResponseAdapter` 的 `sizeFields` 只有 `size/pageSize/limit`（**不含 `page_size`**）。
   与其改全局 config，不如在 `transform.responseAdapter` 里显式解包：
   `{ records: res.data.items, total: res.data.total, current: res.data.page, size: res.data.page_size }`。
   这样业务系统后端的嵌套 envelope `{code,msg,data:{items,total,page,page_size}}` 能正确喂给 ArtTable。

### 端到端验证（CDP 实测）
- 列表 5 行渲染，8 列全部单行不换行 ✓
- 表格与右上角按钮间距正常（ArtTable marginTop:10 生效）✓
- 操作列 2 个 ArtButtonTable 图标按钮（编辑/删除）✓
- 点编辑 → 弹窗「编辑业务系统」，code/name 预填 ✓
- 点新建 → 弹窗「新建业务系统」，6 字段齐全 ✓
- 与角色页截图并排对比：搜索栏/新建按钮/右上角图标/间距/表格/分页器**逐项一致** ✓

### 踩坑
1. **HMR 对 setup 大改 + `h()` formatter 不热更新**：重写后操作列还显示上一版 ElButton 文字按钮，
   整页 reload（ignoreCache）后才生效。调试「改了代码但页面没变」时先硬刷新排除 HMR 缓存。
2. **角色页操作列 `ArtButtonTable` 渲染成 `div.inline-flex + svg`**，不是 `el-button`——
   用 CDP 检查 DOM 时选择器要按 `[class*=inline-flex]` 而非 `button`。

---

## 今日补充（2026-09-14：业务系统 v1.1 — 责任人自由文本+电话+部门下拉）

### 用户反馈（2 个）
1. 搜索栏「名称/编码」仍换行（「码」字换行）
2. 表单调整：部门放责任人上面、部门用**下拉**不是填 ID、责任人改**文本框**
   （责任人可能不是 SOC 平台用户），并新增**责任人联系电话**（安全风险应急联系）

### 关键决策：`owner` 名字被 relationship 占用
模型原有 `owner_id`(FK) + `owner = relationship("User")`。要加自由文本责任人列，
`owner` 已被 relationship 占用 → **改名 `owner`→`owner_user`**（对齐 asset 表的
`owner` 文本列 + `owner_user` relationship 模式），腾名给文本列。
`owner` relationship 仅 `to_dict` 用 `self.owner.username`（1 处），改名成本极低。
`owner_id`/`department_id` 列保留（FK 可选），不再作为表单主推字段。

### 交付（后端 4 + 前端 2）
- `models/business_system.py`：`owner`→`owner_user` relationship；新增
  `owner` VARCHAR(255)（责任人姓名自由文本）+ `owner_contact` VARCHAR(50)（电话）
- 迁移 `v7w8x9y0z1a2`：`ADD COLUMN IF NOT EXISTS owner/owner_contact`（幂等）
- `schemas/business_system.py`：Base/Update/Response 加 owner/owner_contact/department_name
- `api/business_systems.py`：
  - `_to_response` 返回 owner/owner_contact/department_name（list 批量查部门名防 N+1，
    detail/create/update 从 `b.department` relationship 懒加载兜底）
  - **bug 修复**：create 端点构造 `BusinessSystem()` 漏传 `owner/owner_contact`
    （第一版只传老字段，导致 owner 存不进去）→ 补上
- 前端 `api/businessSystem.ts`：类型加 owner/owner_contact/department_name
- 前端 `views/system/business-system/index.vue`：
  - 搜索栏 `labelWidth: '88px'` 修「名称/编码」换行（全局默认 70px 不够 5 字符+斜杠）
  - 表单顺序：编码/名称/重要性/**部门(下拉)**/**责任人(文本)**/**责任人电话(文本)**/描述
  - 部门下拉：`onMounted` 拉 `/api/v1/departments?page_size=200` 组 options（显示名选 id）
  - 列表列：部门(department_name)/责任人(owner)/责任人电话(owner_contact) 替换原 ID 列

### 端到端验证（curl + CDP）
- POST owner=张三 contact=13800138000 dept=公共支撑组 → 201 ✓
- PUT owner=李四 contact=13900139000 dept=市场部 → 200 ✓
- LIST 三字段同步返回 ✓
- CDP：搜索栏 label 单行不换行；表格 9 列（部门/责任人/责任人电话）✓
- 弹窗字段顺序+类型全对：部门[下拉 公共支撑组/市场部/测试部门]、责任人[文本]、责任人电话[文本] ✓

### 踩坑（本轮新增）
1. **模型新增文本列前先 grep relationship 占用**：`owner` 被 relationship 占了，
   直接 `owner = Column(...)` 会冲突报错。改 relationship 名腾出，且与 asset 模式统一。
2. **create 端点易漏传新增字段**：schema 加了字段但端点构造 ORM 时手写字段清单，
   新字段漏传就静默存 NULL。列表查出来才发现（dept_name 有值但 owner None）。
   update 用 `setattr(遍历 payload)` 天然无此问题——create 可考虑也改为 `BusinessSystem(**payload)`。

---

## 今日补充（2026-09-14：业务系统部门下拉空的修复 — page_size 超限 422）

### 问题
用户反馈：新建/编辑业务系统的**部门下拉没有数据**。

### 根因（又一个 envelope 静默假绿）
- 后端 `/api/v1/departments` 的分页参数：`page_size: int = Query(10, ge=1, le=100)` —— 上限 **100**
- 前端 `loadDepartments` 传了 `page_size: 200` → **超上限被 FastAPI 校验拒（422）**
- envelope 中间件把 422 包成 HTTP 200 + `body.data=None`（CLAUDE.md 注意事项 #11 的变体）
- 前端 `res?.data?.items || []` → `None?.items || []` → **`[]` 空数组，不报错**

**诊断路径**（关键：先排除是不是数据问题，再怀疑前端逻辑）：
1. curl 原生 fetch `/departments?page_size=100` → code=200、items=4 正常（接口本身没问题）
2. curl `page_size=200` → `data=None`（复现 422）
3. 临时 console.log 确认 `loadDepartments` 拉到 4 个部门（前端逻辑没问题）
4. dump body 下所有 `.el-select-dropdown` → 部门 dropdown 有 4 个选项（渲染也没问题）
5. 之前抓到 0 是 CDP 抓取时机问题（登录失效/弹窗未开/dropdown 已收），不是数据问题

### 修复
`loadDepartments` 的 `page_size: 200` → `100`（对齐后端上限）。

### 教训（沉淀）
1. **前端 `page_size` 不能超过后端 `Query(le=...)` 上限**：超了会被 422，但 envelope 包成 200，
   前端得 `data=None` 静默空数据。调接口前先查后端分页上限。
2. **下拉类数据源别依赖分页接口的大 page_size**：后端通用 CRUD 接口都有上限（防拖库）。
   若下拉项可能超上限，应加专用的不分页/simple 接口，或前端循环拉全。当前部门 < 100 用 100 够。
3. **「下拉空」先 curl 验证接口，别急着改前端**：本次先怀疑前端逻辑，
   实际根因是参数超限——接口 curl 一下就能看出 data=None。
