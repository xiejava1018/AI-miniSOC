...
BEGIN
    IF EXISTS (
        ...
        RAISE NOTICE '✅ 创建索引: idx_soc_assets_wazuh';
    END IF;
END $$;
...
BEGIN
    IF EXISTS (
        ...
        RAISE NOTICE '✅ 创建索引: idx_soc_assets_type';
    END IF;
END $$;
...
BEGIN
    IF EXISTS (
        ...
        RAISE NOTICE '✅ 创建索引: idx_soc_assets_criticality';
    END IF;
END $$;
...
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_name = 'soc_assets' AND column_name = 'asset_status'
    ) THEN
        CREATE INDEX IF NOT EXISTS idx_soc_assets_status ON soc_assets(asset_status);
        RAISE NOTICE '✅ 创建索引: idx_soc_assets_status';
    END IF;
END $$;
...
BEGIN
    IF EXISTS (
        ...
        RAISE NOTICE '✅ 创建触发器: trigger_update_soc_assets_updated_at';
    END IF;
END $$;
...
BEGIN
    ...
    SELECT EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_name = 'soc_assets' AND column_name = 'wazuh_agent_id'
    ) INTO wazuh_field_exists;

    RAISE NOTICE '✅ 数据库表更新完成！';
    RAISE NOTICE '   soc_assets表字段数: %', column_count;
    RAISE NOTICE '   wazuh_agent_id字段: %', CASE WHEN wazuh_field_exists THEN '✅ 已添加' ELSE '❌ 未添加' END;
