-- ============================================================================
-- AI-miniSOC 数据库迁移脚本 (Bytebase兼容) - 修复版
-- 源数据库: AI-miniSOC-db
...
-- 表: soc_asset_ports
ALTER TABLE soc_asset_ports ADD COLUMN IF NOT EXISTS last_seen timestamp without time zone DEFAULT CURRENT_TIMESTAMP;
...
CREATE INDEX IF NOT EXISTS idx_soc_asset_ports_state ON soc_asset_ports(state);
...
ALTER TABLE soc_assets ADD CONSTRAINT IF NOT EXISTS soc_assets_network_zone_check CHECK (((network_zone)::text = ANY ((ARRAY['intranet'::character varying, 'dmz'::character varying, 'office'::character varying, 'management'::character varying, 'other'::character varying])::text[])));
ALTER TABLE soc_assets ADD CONSTRAINT IF NOT EXISTS soc_assets_data_classification_check CHECK (((data_classification)::text = ANY ((ARRAY['public'::character varying, 'internal'::character varying, 'confidential'::character varying, 'secret'::character varying])::text[])));

ALTER TABLE soc_assets ADD COLUMN IF NOT EXISTS network_zone varchar(50) DEFAULT 'other'::character varying;
ALTER TABLE soc_assets ADD COLUMN IF NOT EXISTS data_classification varchar(20) DEFAULT 'internal'::character varying;
ALTER TABLE soc_assets ADD COLUMN IF NOT EXISTS owner_contact varchar(50);
...
-- 表: soc_menus
ALTER TABLE soc_menus ADD COLUMN IF NOT EXISTS is_visible boolean;
ALTER TABLE soc_menus ADD COLUMN IF NOT EXISTS updated_at timestamp with time zone DEFAULT now();
...
ALTER TABLE soc_roles ADD CONSTRAINT IF NOT EXISTS soc_roles_name_key UNIQUE (name);
...
-- soc_incidents 的外键
ALTER TABLE soc_incidents ADD CONSTRAINT fk_incidents_ai_analysis 
    FOREIGN KEY (ai_analysis_id) REFERENCES soc_ai_analyses(id) ON DELETE SET NULL;

-- soc_asset_incidents 的外键
ALTER TABLE soc_asset_incidents ADD CONSTRAINT fk_asset_incidents_asset 
    FOREIGN KEY (asset_id) REFERENCES soc_assets(id) ON DELETE CASCADE;
ALTER TABLE soc_asset_incidents ADD CONSTRAINT fk_asset_incidents_incident 
    FOREIGN KEY (incident_id) REFERENCES soc_incidents(id) ON DELETE CASCADE;

-- soc_asset_tags 的外键
ALTER TABLE soc_asset_tags ADD CONSTRAINT fk_asset_tags_asset 
    FOREIGN KEY (asset_id) REFERENCES soc_assets(id) ON DELETE CASCADE;

-- soc_audit_logs 的外键
ALTER TABLE soc_audit_logs ADD CONSTRAINT soc_audit_logs_user_id_fkey 
    FOREIGN KEY (user_id) REFERENCES soc_users(id);
ALTER TABLE soc_audit_logs ADD CONSTRAINT soc_audit_logs_session_id_fkey 
    FOREIGN KEY (session_id) REFERENCES soc_user_sessions(id);

-- soc_incident_timeline 的外键
ALTER TABLE soc_incident_timeline ADD CONSTRAINT fk_incident_timeline_incident 
    FOREIGN KEY (incident_id) REFERENCES soc_incidents(id) ON DELETE CASCADE;

-- soc_password_history 的外键
ALTER TABLE soc_password_history ADD CONSTRAINT soc_password_history_user_id_fkey 
    FOREIGN KEY (user_id) REFERENCES soc_users(id) ON DELETE CASCADE;

-- soc_password_reset_tokens 的外键
ALTER TABLE soc_password_reset_tokens ADD CONSTRAINT soc_password_reset_tokens_user_id_fkey 
    FOREIGN KEY (user_id) REFERENCES soc_users(id) ON DELETE CASCADE;

-- soc_rate_limits 的外键
ALTER TABLE soc_rate_limits ADD CONSTRAINT soc_rate_limits_user_id_fkey 
    FOREIGN KEY (user_id) REFERENCES soc_users(id);

-- soc_system_config 的外键
ALTER TABLE soc_system_config ADD CONSTRAINT soc_system_config_updated_by_fkey 
    FOREIGN KEY (updated_by) REFERENCES soc_users(id);

-- soc_user_sessions 的外键
ALTER TABLE soc_user_sessions ADD CONSTRAINT soc_user_sessions_user_id_fkey 
    FOREIGN KEY (user_id) REFERENCES soc_users(id) ON DELETE CASCADE;
